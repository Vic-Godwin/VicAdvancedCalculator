...
last_result = None
power_on = True  # Global flag to check power state
...
def numbers(num):
    ...
    if num == "ON":
        ...
        power_on = True  #Turns Calculator OFF
        return

    if not power_on:
        return # Ignore all input when calculator is OFF

    # Turn OFF
    if num == "OFF":
        display_label.config(text="")
        display_label1.config(text="")
        power_on = False #Turns calculator OFF
        return
    ...
    if num == "=":
        try:
            # Replace Ans with stored last_result before eval
            expression = screen_text.replace("Ans", str(last_result if last_result is not None else "0"))
    ...
    if num in ["+", "-", "*", "/"]:
        if not screen_text and last_result is not None:
            display_label.config(text="Ans" + num)
        else:
...
def resource_path(relative_path):
    ...
    """
    try:
        base_path = sys._MEIPASS  # Temporary folder used by PyInstaller(Can't Forget my Last Error.)
    except Exception:
...
root.iconbitmap(resource_path("vcal.ico"))
...
display_label1 = tk.Label(display_frame, text="0", font=font_large, bg=display_color, fg="black",
